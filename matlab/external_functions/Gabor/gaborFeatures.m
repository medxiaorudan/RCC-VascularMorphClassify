...
for i = 1:u
    for j = 1:v
        ...
        c = c+1;
        gaborReal = real(gaborResult{i,j});
        gaborReal = downsample(gaborReal,d1);
        gaborReal = downsample(gaborReal.',d2);
        gaborReal = reshape(gaborReal.',[],1);
        
        % Normalized to zero mean and unit variance. (if not applicable, please comment this line)
        gaborReal = (gaborReal-mean(gaborReal))/std(gaborReal,1);
        
        featureVector(((c-1)*s+1):(c*s)) = gaborReal;
