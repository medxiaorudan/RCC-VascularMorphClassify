...
window_size = 15; % size of the neighborhood window for the filters
widths = 5:10:45; % same as above; the sigmas should be around the width of the vessels
...
for window_size = (3:7).^2
    
    gaborArray_temp = gaborFilterBank(u, v, window_size, window_size);
    gaborArray = cell(u/2, v);
    for i = 1:u/2
        for j = 1:v
            gaborArray{i, j} = gaborArray_temp{i + u/2, j};
        end
    end
    
    clear gaborArray_temp
    
    featureVector = [featureVector; gaborFeatures(filter_input,gaborArray,1,1)];
